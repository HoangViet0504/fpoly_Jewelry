...
export default function CheckoutContent() {
    ...
    async function GetProvince() {
        try {
            const response = await GetList<{ data: province[] }>(
                "/getAllProvince"
            );
            ...
            try {
                const responseOrder = await RestApi.post("/createOrder", {
                    ...
                    address: `${values.address}, ${
                        listWard.filter(
                            (item) => item.id === Number(values.ward)
                        )[0]._name
                    }, ${
                        listDistrict.filter(
                        ...
                        province.filter(
                            (item2) => item2.id === Number(values.province)
                        )[0]._name
                    }`,
                    note: values.note,
                    discount: discount,
                    ...
                    try {
                        ...
                        if (response) {
                            if (
                                ...
                                values.paymentMethod === "Thanh toán Sepay"
                            ) {
                              navigate(`/payment/${responseOrder.data.data}`);
                              console.log("Thanh toán Sepay");
                                // redirect to payment page
                            } else {
                                try {
                                    const responseUrl = await RestApi.post(
                                        "/checkOut",
                                        {
                                            amount:
                                                calculateSubtotal(cartItem) -
                                    ...
                                    window.location.href =
                                        responseUrl.data.data.order_url;
                                } catch (error) {
                                    console.log(error);
                                }
                            }
